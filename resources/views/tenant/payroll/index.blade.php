        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-gray-600 text-sm font-medium">Monthly Payroll</p>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-gray-600 text-sm font-medium">Pending Payrolls</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('tenant.payroll.reports.tax-report', $tenant) }}"
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div class="space-y-3">
                    <a href="{{ route('tenant.payroll.employees.index', $tenant) }}"
                    ...
                    <a href="{{ route('tenant.payroll.departments.index', $tenant) }}"
                    ...
                    <a href="{{ route('tenant.payroll.components.index', $tenant) }}"
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                                <div class="flex items-center">
                                    <div class="w-3 h-3 rounded-full mr-4
                                        @if($payroll->status === 'completed') bg-emerald-500
                                        @elseif($payroll->status === 'processing') bg-orange-500
                                        @elseif($payroll->status === 'approved') bg-blue-500
                                        @else bg-gray-400 @endif"></div>
                                    <div>
                                        <h4 class="font-semibold text-gray-900">{{ $payroll->name ?? $payroll->period_name }}</h4>
                                        <p class="text-gray-600 text-sm">{{ $payroll->created_at->format('M d, Y') }} • {{ ucfirst($payroll->status) }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="font-semibold text-gray-900">₦{{ number_format($payroll->total_gross ?? 0, 2) }}</p>
                                    <p class="text-gray-600 text-sm">{{ $payroll->employee_count ?? 0 }} employees</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
